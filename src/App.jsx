...
function App() {
  ...
  const saveDateApplied = async (jobId) => {
    try {
      const response = await fetch(`http://localhost:5001/api/jobs/${jobId}`, {
        ...
        body: JSON.stringify({ date_applied: editDateValue || 'Pending' }),
      });
  ...
  const deleteSelectedJobs = () => {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        setConfirmDialog(prev => ({ ...prev, isOpen: false }));
        try {
          const response = await fetch(`http://localhost:5001/api/jobs/bulk-delete`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const jobsUrl = `http://localhost:5001/api/jobs?sortBy=${sortBy}&sortOrder=${sortOrder}&search=${encodeURIComponent(searchTerm)}&status=${statusFilter}&connectStatus=${connectStatusFilter}&minScore=${minScoreFilter}&eligibility=${eligibilityFilter}`;
      const statsUrl = `http://localhost:5001/api/stats`;
